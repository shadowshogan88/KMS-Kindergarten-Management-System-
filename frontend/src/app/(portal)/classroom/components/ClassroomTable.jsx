...
const ClassroomTable = () => {
  ...
  const load = async nextPage => {
    ...
    setError('');
    try {
      const data = await apiJson(`/rooms/?page=${nextPage}`);
  ...
  const openAddModal = () => {
    setEditing(null);
    requestAnimationFrame(() => openOverlay('#classroom-edit-modal'));
  };
  ...
  const openDeleteModal = row => {
    setSelected(row);
    requestAnimationFrame(() => openOverlay('#classroom-delete-modal'));
  };

  const onCreate = async payload => {
    const created = await apiJson('/rooms/', { method: 'POST', body: payload });
    setItems(prev => [created, ...prev].slice(0, 10));
    setCount(prev => prev + 1);
    setFlash('Classroom added successfully.');
  };
  const onUpdate = async (row, payload) => {
    const updated = await apiJson(`/rooms/${row.id}/`, { method: 'PATCH', body: payload });
    setItems(prev => prev.map(d => (d.id === row.id ? updated : d)));
    setFlash('Classroom updated successfully.');
  };
  const onDelete = async () => {
    if (!selected?.id) return;
    await apiJson(`/rooms/${selected.id}/`, { method: 'DELETE' });
    setItems(prev => prev.filter(d => d.id !== selected.id));
    setSelected(null);
    setCount(prev => Math.max(0, prev - 1));
    setFlash('Classroom deleted successfully.');
  };
    ...
    <div className="card">
      <div className="card-header flex justify-between items-center">
        ...
        <button
          ...
          type="button"
        >
          <LuPlus className="size-4" /> Add Classroom
        </button>
      </div>
      ...
      <div className="flex flex-col">
        ...
        {!authStorage.getAccess() ? (
          <div className="px-5 py-4 text-sm text-default-600">
            Please sign in from <Link className="text-primary underline" to="/portal">Portal</Link> to load classrooms from
            backend.
          </div>
        ) : null}
              ...
              <table className="min-w-full divide-y divide-default-200">
                <thead className="font-semibold whitespace-nowrap">
                  <tr className="text-sm text-default-800 divide-x divide-default-200">
                    ...
                    <th className="px-3.5 py-3 text-start">Room No</th>
                ...
                <tbody className="divide-y divide-default-200">
                  ...
                  {items.map(row => (
                    <tr key={row.id} className="text-default-800 font-normal whitespace-nowrap divide-x divide-default-200">
                      ...
                      <td className="px-3.5 py-3 text-sm">{row.room_no}</td>
                      <td className="px-3.5 py-3 text-sm">{row.capacity}</td>
                      <td className="px-3.5 py-3">
                        <div className="flex items-center gap-2">
                          <button
                            ...
                            aria-expanded="false"
                            aria-controls="classroom-edit-modal"
                          >
                          ...
                          <button
                            ...
                            aria-haspopup="dialog"
                            aria-expanded="false"
                            aria-controls="classroom-delete-modal"
                          >
      ...
      <DeleteModal classroom={selected} onConfirm={async () => {
        await onDelete();
        if (items.length === 1 && page > 1) await load(page - 1);
        else await load(page);
      }} />
      <AddClassroom room={editing} onCreated={async p => {
        await onCreate(p);
        await load(1);
      }} onUpdated={async (d, p) => {
        await onUpdate(d, p);
