...
const SectionTable = () => {
  ...
  const load = async nextPage => {
    ...
    setError('');
    try {
      const data = await apiJson(`/sections/?page=${nextPage}`);
  ...
  const openAddModal = () => {
    setEditing(null);
    requestAnimationFrame(() => openOverlay('#section-edit-modal'));
  };
  ...
  const openDeleteModal = row => {
    setSelected(row);
    requestAnimationFrame(() => openOverlay('#section-delete-modal'));
  };

  const onCreate = async payload => {
    const created = await apiJson('/sections/', { method: 'POST', body: payload });
    setItems(prev => [created, ...prev].slice(0, 10));
    setCount(prev => prev + 1);
    setFlash('Section added successfully.');
  };

  const onUpdate = async (row, payload) => {
    const updated = await apiJson(`/sections/${row.id}/`, { method: 'PATCH', body: payload });
    setItems(prev => prev.map(d => (d.id === row.id ? updated : d)));
    setFlash('Section updated successfully.');
  };

  const onDelete = async () => {
    if (!selected?.id) return;
    await apiJson(`/sections/${selected.id}/`, { method: 'DELETE' });
    setItems(prev => prev.filter(d => d.id !== selected.id));
    setSelected(null);
    setCount(prev => Math.max(0, prev - 1));
    setFlash('Section deleted successfully.');
  };
    ...
    <div className="card">
      <div className="card-header flex justify-between items-center">
        ...
        <button
          ...
          type="button"
        >
          <LuPlus className="size-4" /> Add Section
        </button>
      </div>
      ...
      <div className="flex flex-col">
        ...
        {!authStorage.getAccess() ? (
          <div className="px-5 py-4 text-sm text-default-600">
            Please sign in from <Link className="text-primary underline" to="/portal">Portal</Link> to load sections from
            backend.
          </div>
        ) : null}
                      ...
                      <td className="px-3.5 py-3">
                        <div className="flex items-center gap-2">
                          <button
                            ...
                            aria-expanded="false"
                            aria-controls="section-edit-modal"
                          >
                          ...
                          <button
                            ...
                            aria-haspopup="dialog"
                            aria-expanded="false"
                            aria-controls="section-delete-modal"
                          >
      ...
      <DeleteModal section={selected} onConfirm={async () => {
        await onDelete();
        if (items.length === 1 && page > 1) await load(page - 1);
        else await load(page);
      }} />
      <AddSection section={editing} onCreated={async p => {
        await onCreate(p);
        await load(1);
      }} onUpdated={async (d, p) => {
        await onUpdate(d, p);
