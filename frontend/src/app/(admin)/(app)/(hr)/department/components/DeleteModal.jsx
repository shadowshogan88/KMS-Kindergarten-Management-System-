...
const closeDepartmentDeleteOverlay = () => closeOverlay('#department-delete-modal');

const DeleteModal = ({ department, onConfirm }) => {
      ...
      <div className="hs-overlay-animation-target hs-overlay-open:scale-100 hs-overlay-open:opacity-100 scale-95 opacity-0 ease-in-out transition-all duration-200 md:w-sm m-3 mx-auto min-h-[calc(100%-56px)] flex items-center">
        <div className="w-full flex flex-col card shadow-2xs border border-default-200 rounded-xl pointer-events-auto px-6 py-8 relative">
          <div className="absolute top-3 end-3">
            <button type="button" className="size-5 text-default-800" aria-label="Close" data-hs-overlay="#department-delete-modal" onClick={closeDepartmentDeleteOverlay} disabled={isSubmitting}>
          ...
          <h3 className="font-semibold text-base text-default-800 text-center">
            ...
            <div className="mt-5 text-center">
              ...
              <p className="text-default-500 text-sm font-normal">
                Delete department{department?.name ? ` “${department.name}”` : ''}?
              </p>
              ...
              <div className="mt-5 flex gap-2 justify-center">
                <button data-hs-overlay="#department-delete-modal" onClick={closeDepartmentDeleteOverlay} className="btn text-danger bg-transparent hover:bg-danger/10" aria-label="Close" disabled={isSubmitting}>
                  Cancel
                </button>
                <button className="bg-danger text-white btn border-0 btn-sm" onClick={submit} disabled={isSubmitting || !department}>
                  Yes,Delete It!
                </button>
              </div>
            </div>
